...
func Import(ctx context.Context, root, sourcePath string, expected Digest) error {
	...
	if !validAbsolutePath(root) || !validAbsolutePath(sourcePath) {
	...
	sourceFD, err := unix.Open(sourcePath, unix.O_RDONLY|unix.O_NOFOLLOW|unix.O_CLOEXEC|unix.O_NONBLOCK, 0)
	if err != nil {
		return storeDiagnostic(expected, IO, sourcePath, "open import source", err)
	}
	var sourceStatus unix.Stat_t
	if err := unix.Fstat(sourceFD, &sourceStatus); err != nil {
		_ = unix.Close(sourceFD)
		return storeDiagnostic(expected, IO, sourcePath, "inspect import source", err)
	}
	if sourceStatus.Mode&unix.S_IFMT != unix.S_IFREG {
		_ = unix.Close(sourceFD)
	...
	stageFD, stageName, err := createStage(directory, rand.Reader)
	if err != nil {
...
func loadCandidate(ctx context.Context, directory int, path, name string, digest Digest) (Source, error) {
	fd, err := openRegularAt(directory, name)
	if err != nil {
		if errors.Is(err, unix.ENOENT) {
			return Source{}, err
		}
