...
func probeZypper(ctx context.Context, effect effects) candidate {
	...
	if err != nil {
		return systemIndeterminate(backend, fmt.Sprintf("identify zypper: %v", err))
	}
	rpm, err := effect.identify(rpmPath)
	if err != nil {
		return systemIndeterminate(backend, fmt.Sprintf("identify rpm companion: %v", err))
	}

	result, err := effect.run(ctx, zypper, []string{"--version"}, nil)
	if err != nil || !result.Started || result.ExitCode != 0 {
		return systemIndeterminate(backend, nativeDiagnostic("probe zypper", result, err))
	}
	...
	if err != nil || !result.Started || result.ExitCode != 0 {
		return systemIndeterminate(backend, nativeDiagnostic("probe rpm companion", result, err))
	}
...
func parseZypperRoots(data []byte) ([]string, error) {
	...
	for scanner.Scan() {
		...
		default:
			...
			if len(columns) != 5 || columns[0] != "i+" || columns[1] == "" ||
...
func resolveZypperDemand(name string, roots map[string]struct{}, result linux.Result, runErr error) (demandState, error) {
...
func (behavior zypperBehavior) Preview(ctx context.Context, evidence proof, observation Observation) (Offer, error) {
	...
	desired := unresolvedFrom(observation)
	if len(desired) == 0 {
		return newOffer(nil)
	}
...
func unresolvedFrom(observation Observation) []string {
	demands := observation.demands()
	desired := make([]string, 0, len(demands))
	for _, demand := range demands {
		if demand.State != demandDirect {
			desired = append(desired, demand.Name)
		}
	}
	return desired
}
...
func zypperEdition(value string) string {
...
func (behavior zypperBehavior) Commit(ctx context.Context, evidence proof, observation Observation, expected Offer) (commitResult, error) {
	...
	result, err := behavior.effects.run(ctx, native.zypper, zypperTransactionArgs(false, unresolvedFrom(observation)), nil)
...
func (zypperBehavior) Verify(before Observation, offer Offer, after Observation) error {
	return verifyRPMTransition("Zypper", before, offer, after)
}
