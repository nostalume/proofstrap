...
func linuxEffects() effects { return effects{identify: linux.Identify, run: linux.Run} }
...
func systemIndeterminate(backend binding.PackageBackendID, detail string) candidate {
...
// SelectHost reduces fixed built-in system-manager evidence without distro dispatch.
func SelectHost(ctx context.Context) Selection {
	...
	return selectHost([]candidate{
		...
		probeAPK(ctx, effect, systemAPKFiles()),
	})
}
...
func SelectExact(ctx context.Context, backend binding.PackageBackendID) Selection {
	...
	case "apk":
		value = probeAPK(ctx, effect, systemAPKFiles())
	default:
