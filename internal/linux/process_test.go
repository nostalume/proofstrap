...
func identify(t *testing.T, path string) linux.Identity {
	t.Helper()
	identity, err := linux.Identify(path)
	if err != nil {
...
func TestIdentifyRejectsRelativeAndUntrustedPaths(t *testing.T) {
	if _, err := linux.Identify("bin/sh"); err == nil {
	...
	if _, err := linux.Identify(path); err == nil {
		t.Fatal("executable below untrusted parent admitted")
	}
	if _, err := linux.Identify("/etc/passwd"); err == nil {
...
func TestRunPassesArgumentsAndInputLiterally(t *testing.T) {
	...
	result, err := linux.Run(deadlineContext(t), printf, []string{"%s", "$HOME;*"}, nil)
	...
	result, err = linux.Run(deadlineContext(t), cat, nil, []byte("first\nsecond\n"))
...
func TestRunUsesOnlyFixedEnvironment(t *testing.T) {
	...
	result, err := linux.Run(deadlineContext(t), identify(t, "/usr/bin/env"), nil, nil)
...
func TestRunReturnsNormalNonzeroExitAsResult(t *testing.T) {
	result, err := linux.Run(deadlineContext(t), identify(t, "/bin/sh"), []string{"-c", "printf disabled; exit 7"}, nil)
...
func TestRunRequiresDeadlineAndRejectsIdentityDriftBeforeStart(t *testing.T) {
	...
	result, err := linux.Run(context.Background(), identity, nil, nil)
	if err == nil || result.Started {
		t.Fatalf("deadline-free result=%#v err=%v", result, err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	cancel()
	result, err = linux.Run(ctx, identity, nil, nil)
	if !errors.Is(err, context.Canceled) || result.Started {
		t.Fatalf("canceled result=%#v err=%v", result, err)
	}

	identity.Digest[0] ^= 0xff
	result, err = linux.Run(deadlineContext(t), identity, nil, nil)
	if !errors.Is(err, linux.ErrIdentityChanged) || result.Started {
		t.Fatalf("drift result=%#v err=%v", result, err)
	}

	result, err = linux.Run(deadlineContext(t), linux.Identity{Path: "/proofstrap-missing-executable"}, nil, nil)
	if err == nil || errors.Is(err, linux.ErrIdentityChanged) || result.Started {
...
func TestRunTimeoutAndSignalArePostStartErrors(t *testing.T) {
	...
	defer cancel()
	started := time.Now()
	result, err := linux.Run(ctx, identify(t, "/bin/sh"), []string{"-c", "sleep 10 & wait"}, nil)
	...
	result, err = linux.Run(deadlineContext(t), identify(t, "/bin/sh"), []string{"-c", "kill -TERM $$"}, nil)
	if err == nil || !result.Started {
...
func TestRunCaptureLimitIsExactAndOverflowFailsClosed(t *testing.T) {
	...
	result, err := linux.Run(deadlineContext(t), head, []string{"-c", "8388608", "/dev/zero"}, nil)
	...
	result, err = linux.Run(deadlineContext(t), head, []string{"-c", "8388609", "/dev/zero"}, nil)
	if !errors.Is(err, linux.ErrOutputLimit) || !result.Started || len(result.Stdout) != captureLimit {
		t.Fatalf("overflow bytes=%d result=%#v err=%v", len(result.Stdout), result, err)
	}

	shell := identify(t, "/bin/sh")
	result, err = linux.Run(deadlineContext(t), shell, []string{"-c", "head -c 8388609 /dev/zero >&2"}, nil)
	if !errors.Is(err, linux.ErrOutputLimit) || !result.Started || len(result.Stderr) != captureLimit {
