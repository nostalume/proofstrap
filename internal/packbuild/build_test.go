...
func TestCreateStageHasBoundedDeterministicCollisions(t *testing.T) {
	...
	if stageFD, _, err := createStage(fd, bytes.NewReader(bytes.Repeat(token, 16))); err == nil || stageFD >= 0 {
		t.Fatalf("sixteen collisions = fd %d, error %v", stageFD, err)
	}
	available := bytes.Repeat([]byte{0x2b}, 16)
	stageFD, stageName, err := createStage(fd, io.MultiReader(bytes.NewReader(token), bytes.NewReader(available)))
	if err != nil {
		t.Fatal(err)
	}
