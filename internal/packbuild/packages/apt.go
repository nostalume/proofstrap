...
type aptProof struct {
	get          linuxexec.Identity
	getVersion   string
	query        linuxexec.Identity
	queryVersion string
	mark         linuxexec.Identity
	markVersion  string
	dpkg         linuxexec.Identity
	dpkgVersion  string
	nativeArch   string
}
...
func probeApt(ctx context.Context, effect effects) candidate {
	...
	identities := make([]linuxexec.Identity, len(paths))
	for index, path := range paths {
		...
		if err != nil {
			return aptIndeterminate(backend, fmt.Sprintf("identify Apt companion %s: %v", path, err))
		}
	}
	...
	for index, identity := range identities {
		...
		if runErr != nil || !result.Started || result.ExitCode != 0 || len(result.Stderr) != 0 {
			return aptIndeterminate(backend, nativeDiagnostic("probe "+paths[index], result, runErr))
		}
		aptTool := index == 0 || index == 2
		versions[index], err = parseAptToolVersion(result.Stdout, aptTool)
		if err != nil {
			return aptIndeterminate(backend, err.Error())
		}
	...
	if versions[0] != versions[2] || versions[1] != versions[3] {
		return aptIndeterminate(backend, "Apt companion versions disagree")
	}
	result, runErr := effect.run(ctx, identities[3], []string{"--print-architecture"}, nil)
	if runErr != nil || !result.Started || result.ExitCode != 0 {
		return aptIndeterminate(backend, nativeDiagnostic("probe native architecture", result, runErr))
	}
...
func aptIndeterminate(backend binding.PackageBackendID, detail string) candidate {
