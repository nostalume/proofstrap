...
func probeDNF4(ctx context.Context, effect effects) candidate {
	...
	identities := make([]linuxexec.Identity, 0, 2)
	for _, path := range []string{dnf4Path, dnfPath} {
		...
		case identifyErr != nil:
			return dnf4Indeterminate(backend, fmt.Sprintf("identify DNF4 alias %s: %v", path, identifyErr))
		}
		if len(identities) != 0 && identities[0] != identity {
			return dnf4Indeterminate(backend, "DNF4 aliases resolve to different executable identities")
		}
		if len(identities) == 0 {
	...
	if runErr != nil || !result.Started || result.ExitCode != 0 || len(result.Stderr) != 0 {
		return dnf4Indeterminate(backend, nativeDiagnostic("probe DNF4", result, runErr))
	}
	version, supported, parseErr := parseDNF4Version(result.Stdout)
	if parseErr != nil {
		return dnf4Indeterminate(backend, parseErr.Error())
	}
	if !supported {
...
func dnf4Indeterminate(backend binding.PackageBackendID, detail string) candidate {
	return candidate{evidence: candidateEvidence{
...
func (dnf4Behavior) Verify(before Observation, offer Offer, after Observation) error {
	...
	for _, delta := range deltas {
		...
		if !ok {
			return fmt.Errorf("malformed reviewed DNF4 package key")
		}
		...
		case Add:
			...
			if delta.Before() != "" || existed || !hasNew {
				return fmt.Errorf("DNF4 Add for %q does not match post-observation", delta.Key())
			}
			delete(current, newKey)
		case Upgrade:
			if !hadOld || !hasNew || old.State != new.State {
				return fmt.Errorf("DNF4 Upgrade for %q does not match post-observation", delta.Key())
			}
			delete(prior, oldKey)
			delete(current, newKey)
		}
	}
	return equalRemainingRecords("DNF4", prior, current)
}
