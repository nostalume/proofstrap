...
func probeDNF5(ctx context.Context, effect effects, files dnf5Files) candidate {
	...
	if err != nil {
		return dnf5Indeterminate(backend, fmt.Sprintf("identify dnf5: %v", err))
	}
	result, err := effect.run(ctx, executable, []string{"--version"}, nil)
	if err != nil || !result.Started || result.ExitCode != 0 || len(result.Stderr) != 0 {
		return dnf5Indeterminate(backend, nativeDiagnostic("probe dnf5", result, err))
	}
	version, supported, err := parseDNF5Version(result.Stdout)
	if err != nil {
		return dnf5Indeterminate(backend, err.Error())
	}
	if !supported {
	...
	if files.mkdirTemp == nil || files.readBounded == nil || files.removeAll == nil {
		return dnf5Indeterminate(backend, "invalid dnf5 filesystem effects")
	}
	return candidate{
...
func dnf5Indeterminate(backend binding.PackageBackendID, detail string) candidate {
	return candidate{evidence: candidateEvidence{backend: backend, role: SystemCandidate, state: candidateIndeterminate, detail: detail}}
}
...
func (dnf5Behavior) Verify(before Observation, offer Offer, after Observation) error {
	...
	for _, delta := range deltas {
		...
		if !ok {
			return fmt.Errorf("malformed reviewed dnf5 package key")
		}
		...
		case Add:
			...
			if delta.Before() != "" || existed || !hasNew {
				return fmt.Errorf("dnf5 Add for %q does not match post-observation", delta.Key())
			}
			delete(current, newKey)
		case Upgrade:
			if !hadOld || !hasNew || old.State != new.State {
				return fmt.Errorf("dnf5 Upgrade for %q does not match post-observation", delta.Key())
			}
			delete(prior, oldKey)
			delete(current, newKey)
		}
	}
	return equalRemainingRecords("dnf5", prior, current)
}
