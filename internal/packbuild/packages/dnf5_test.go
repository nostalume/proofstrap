...
func TestDNF5AdmissionBindsExactExecutableAndMajorVersion(t *testing.T) {
	identity := linuxexec.Identity{Path: dnf5Path, Digest: [32]byte{1}}
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()

	script := newDNF5Script(t, map[string]linuxexec.Identity{dnf5Path: identity}, []dnf5Run{{
		identity: identity,
		args:     []string{"--version"},
	...
	unsupported := newDNF5Script(t, map[string]linuxexec.Identity{dnf5Path: identity}, []dnf5Run{{
	...
	malformed := newDNF5Script(t, map[string]linuxexec.Identity{dnf5Path: identity}, []dnf5Run{{
...
func newDNF5Script(t *testing.T, identities map[string]linuxexec.Identity, runs []dnf5Run) *dnf5Script {
	t.Helper()
...
func TestDNF5ObserveClassifiesInstalledRootsInOneQuery(t *testing.T) {
	proof := dnf5Proof{executable: linuxexec.Identity{Path: dnf5Path, Digest: [32]byte{1}}, version: "5.2.16.0"}
...
func TestDNF5ObserveRejectsNonConcreteDesiredNameBeforeNativeRun(t *testing.T) {
	proof := dnf5Proof{executable: linuxexec.Identity{Path: dnf5Path, Digest: [32]byte{1}}, version: "5.2.16.0"}
	runs := 0
	behavior := dnf5Behavior{effects: effects{run: func(context.Context, linuxexec.Identity, []string, []byte) (linuxexec.Result, error) {
		runs++
		return started(""), nil
	}}}
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
...
func TestDNF5PreviewCleansAfterNativeReadAndCleanupFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{"native", dnf5Run{identity: proof.executable, result: linuxexec.Result{Started: true, ExitCode: 1}}, &dnf5FileScript{directory: "/tmp/proofstrap-dnf5-native"}, 0},
...
func TestDNF5CommitStartedMeansReplayStarted(t *testing.T) {
	...
	tests := []struct {
		name        string
		store       linuxexec.Result
		replay      *dnf5Run
		wantStarted bool
	}{
		{"store-failure", linuxexec.Result{Started: true, ExitCode: 1}, nil, false},
		{"replay-exit", started("stored\n"), &dnf5Run{identity: proof.executable, result: linuxexec.Result{Started: true, ExitCode: 1}}, true},
		{"replay-transport", started("stored\n"), &dnf5Run{identity: proof.executable, result: linuxexec.Result{Started: true}, err: context.DeadlineExceeded}, true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
