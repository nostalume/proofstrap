...
func TestAptAdapterObservesPreviewsAndCommitsThroughNativeEvidence(t *testing.T) {
	...
	runs := []aptRun{
		...
		{proof.dpkg, []string{"--compare-versions", "1.0-1", "eq", "1.1-1"}, linuxexec.Result{Started: true, ExitCode: 1}, nil},
		{proof.get, aptTransactionArgs(true, aptRefs(t, "amd64", "bash", "dep", "newpkg")), startedBytes(aptFixture(t, "preview.txt")), nil},
		{proof.dpkg, []string{"--compare-versions", "1.0-1", "eq", "1.1-1"}, linuxexec.Result{Started: true, ExitCode: 1}, nil},
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			script := newAptScript(t, nil, []aptRun{{
				...
				linuxexec.Result{Started: true, Stdout: []byte(test.output), Stderr: []byte(test.stderr)}, nil,
			}})
			ctx, cancel := context.WithTimeout(context.Background(), time.Second)
			defer cancel()
...
func TestAptCommitPreservesStarted(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{linuxexec.Result{Started: true, ExitCode: 100}, nil},
		{linuxexec.Result{Started: true}, context.DeadlineExceeded},
		{linuxexec.Result{}, nil},
	} {
		script := newAptScript(t, nil, []aptRun{
...
func testAptProof() aptProof {
	return aptProof{
		get: linuxexec.Identity{Path: aptGetPath, Digest: [32]byte{1}}, getVersion: "2.6.1",
		query: linuxexec.Identity{Path: dpkgQueryPath, Digest: [32]byte{2}}, queryVersion: "1.21.22",
		mark: linuxexec.Identity{Path: aptMarkPath, Digest: [32]byte{3}}, markVersion: "2.6.1",
		dpkg: linuxexec.Identity{Path: dpkgPath, Digest: [32]byte{4}}, dpkgVersion: "1.21.22", nativeArch: "amd64",
	}
}

func started(text string) linuxexec.Result { return startedBytes([]byte(text)) }
func startedBytes(data []byte) linuxexec.Result {
	return linuxexec.Result{Started: true, Stdout: data}
}
...
func newAptScript(t *testing.T, identities map[string]linuxexec.Identity, runs []aptRun) *aptScript {
	t.Helper()
