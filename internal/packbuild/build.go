...
func Build(ctx context.Context, inputRoot, outputPath string) (pack.Digest, error) {
	...
	if !validPath(inputRoot) || !validPath(outputPath) {
	...
	rootFD, err := openDirectory(inputRoot)
	if err != nil {
	...
	contentFD, err := unix.Openat(rootFD, contentName, unix.O_RDONLY|unix.O_DIRECTORY|unix.O_NOFOLLOW|unix.O_CLOEXEC, 0)
	if err != nil {
	...
	parentFD, err := openDirectory(parentPath)
	if err != nil {
	...
	stageFD, stageName, err := createStage(parentFD, rand.Reader)
	if err != nil {
